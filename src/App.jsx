...
const REQUIRED_DOCS = [
    ...
    { id: 'pernyataan_atlet', label: 'SURAT PERNYATAAN OLAHRAGAWAN', categories: ['Olahragawan'] },
...
export default function App() {
    ...
    if (view === 'admin') return <AdminDashboard data={pesertaList} onLogout={handleLogout} onUpdateStatus={handleUpdateStatus} onResetPassword={handleResetPassword} logos={logos} setLogos={setLogos} bgImages={bgImages} setBgImages={setBgImages} onChangeAdminPassword={setAdminPassword} passwords={passwords} />;
                    ...
                    <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 scroll-smooth">
                        ...
                        {formData.kategori === 'Olahragawan' && (
                            <div className="bg-gradient-to-br from-blue-50 to-indigo-50/30 p-6 md:p-8 rounded-2xl border border-blue-100/50 shadow-sm">
                                ...
                                {/* --- TAMPILAN INFORMASI ATLET YANG BARU & RAPI --- */}
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                    <div className="lg:col-span-2">
                                    ...
                                    <div className="lg:col-span-2 md:col-span-1">
                                        ...
                                        <input type="text" name="bpjsTk" value={formData.bpjsTk} onChange={handleChange} readOnly={isReadOnly} className={`${inputClasses} font-mono`} placeholder="Contoh: 19027791094" />
                                    </div>
                                </div>
                            </div>
                        )}
...
// ==========================================
function AdminDashboard({ data, onLogout, onUpdateStatus, onResetPassword, logos, setLogos, bgImages, setBgImages, onChangeAdminPassword, passwords }) {
    ...
    const handleBgUpload = (e, kategori) => {
        const file = e.target.files[0];
        if (file) {
            const reader = new FileReader();
            reader.onloadend = () => {
                setBgImages(prev => ({ ...prev, [kategori]: reader.result }));
            };
            reader.readAsDataURL(file);
        }
    };
                        ...
                        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/40 border border-slate-200/60 overflow-hidden flex flex-col">
                            ...
                            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="w-full">
                                    ...
                                    {logos[0] && <div className="text-center mt-3"><button onClick={() => {const l = [...logos]; l[0] = null; setLogos(l);}} className="text-xs font-bold text-rose-500 hover:text-rose-600 bg-rose-50 px-4 py-2 rounded-lg transition-colors tracking-wider">Hapus Logo Utama</button></div>}
                                </div>
                                <div className="w-full">
                                    ...
                                    {logos[1] && <div className="text-center mt-3"><button onClick={() => {const l = [...logos]; l[1] = null; setLogos(l);}} className="text-xs font-bold text-rose-500 hover:text-rose-600 bg-rose-50 px-4 py-2 rounded-lg transition-colors tracking-wider">Hapus Logo Sekunder</button></div>}
                                </div>
                            </div>
                                    ...
                                    <div key={kategori} className="w-full">
                                        ...
                                        {bgImages[kategori] && (
                                            <div className="text-center mt-3">
                                                <button onClick={() => setBgImages(prev => ({...prev, [kategori]: null}))} className="text-xs font-bold text-rose-500 hover:text-rose-600 bg-rose-50 px-4 py-2 rounded-lg transition-colors tracking-wider shadow-sm border border-rose-100 hover:-translate-y-0.5">
                                                    Hapus Background
                                                </button>
                                            </div>
                                        )}
